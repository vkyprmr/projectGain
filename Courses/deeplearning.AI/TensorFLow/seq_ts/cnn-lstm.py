'''
Developer: vkyprmr
Filename: cnn-lstm.py
Created on: 2020-09-12 at 02:21:13
'''
'''
Modified by: vkyprmr
Last modified on: 2020-09-12 at 03:15:50
'''
...
results = predict(series)
 """
def predict(model, series, window_size):
    ds = tf.data.Dataset.from_tensor_slices(series)
...
results = predict(model, series, window_size)
preds = results[split_time - window_size:-1, -1, 0]

plot_series(time_valid, x_valid)
plot_series(time_valid, preds)

print(f'MAE: {mean_absolute_error(x_valid, preds).numpy()}')
